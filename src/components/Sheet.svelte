...
{#if open}
  <Portal target=".sheets">
    <div
      use:stackable
      class={`fixed top-0 left-0 w-screen h-screen overflow-hidden  overlay bg-opacity-60 ${
    ...
    >
      <div
        class={`fixed bottom-0 left-0 right-0 w-full transition-all duration-200 mx-auto bg-bluegray modal rounded-t-4xl overflow-hidden`}
        in:fly={{
          duration: 300,
          delay: 50,
      ...
      >
        {#if color}
          <div
            class={`absolute top-0 left-0 bg-gradient-to-b pointer-events-none ${colors[
              color
            ].join(" ")} h-56 w-full z-10`}
          />
        {/if}
        <div
          class={`absolute bg-bluegray top-0 left-0 right-0 z-30 w-full mx-auto transition-all duration-150 ${
            scrolled ? "pt-1 bg-transparent opacity-50 scrolled " : "pt-16"
          }`}
          bind:this={headerElm}
        >
          <button
            on:click={toggle}
            class={`absolute transition-all duration-150 top-0 right-0 w-10 h-10 mr-16 text-bluegray-800 ${
              scrolled ? "mt-5" : "mt-16"
            }`}><X /></button
          >
          {#if context}
        ...
        <div
          ...
          class="relative z-20 h-full overflow-auto content"
          style={`padding-top: ${
            headerElm ? headerElm.offsetTop + headerElm.clientHeight + 12 : 0
          }px`}
        >
          <slot />
          {#if $$slots.footer}
