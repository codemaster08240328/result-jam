...
<div class={`relative ${className}`}>
  ...
  <div class="tar_user-wrapper" style={`width: ${getCompletePercent(user)}%`}>
    <Tooltip position={getPosition(user)} className="top-1" show>
      <div slot="tooltip-content">
        ...
        <p class="font-semibold text-black text-sm mt-2">
          1 Live Conversation Generated
        </p>
        <div class="text-black font-normal text-xs flex mt-1">
          <div class="bg-green w-5 h-4 polygon-clip-path-1" />
          <span
            class="ml-1.5">{user.numStepsComplete - CUR_USER.numStepsComplete}
            Steps ahead of you</span>
        </div>
      </div>
      <Avatar
        slot="tooltip-target"
    ...
    <span
      class="ml-1.5 text-white italic text-xl font-bold">{getCompletePercent(user)}%</span>
  </div>
</div>
